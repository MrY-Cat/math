 ...
 *
 * 迭代规则：根据当前组合结果，按照"从右往左首个小于n-j(j为该元素右侧元素数)的元素b增加1，同时b右侧的所有元素从左到右依次变为其左侧元素+1"得到下一个组合结果.
 * @return 从[n]个元素(1,2,..,n)中选[r]个的组合的结果矩阵，该矩阵共有[C] ([n],[r])行，[r]列，每行存储一种组合结果，每种结果中的元素都自小到大排列.
 * @throws MathIllegalException 当n或r为负整数时.
